...
- **X軸**: 右方向が正
- **Y軸**: 下方向が正
- **単位**: 1.0 = 1ピクセル（Sprite/テキストの位置・サイズはピクセル基準）
- **テキストの位置**: `TextRenderer::DrawText` の `position` は「行の上端」を基準
- **カメラ移動（W/S）**: Wで上（$y$減少）、Sで下（$y$増加）

座標系の変更は `Camera2D` の正射影行列で行います。独自の座標系を使いたい場合は、
`Camera2D::UpdateProjectionMatrix()` を調整してください。
